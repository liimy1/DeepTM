...
Pssm_Path = './Data/pssm/'
Hhm_Path = './Data/PSSM_HHM_A3M/hhm/'
Spd33_Path = './Data/spd33/'

Node_Feature_num = 121
...
def get_matrix():
    for uniprot_id in tqdm(fastalist):
        ...
        matrix = np.concatenate([blosum,pssm,hhm,PP7,np.array(AAfq).reshape(-1,1),np.array(dipfq)],axis=1)
        np.save('./Data/node_features_OGT/' + uniprot_id + '.npy',matrix)
...
def cal_mean_std():
    ...
    for name in tqdm(fastalist):
        matrix = np.load('./Data/node_features_OGT/' + name+'.npy')
